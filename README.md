...
打开浏览器本地页（默认 `http://127.0.0.1:8765/`，端口占用则换随机端口）。默认只问三件事：谁、要做什么、能不能做。策略默认用自带的 `policies/example.yaml`，检查结果会自动写入审计。只有「要两个人批」时才出现两个「同意」按钮（默认 alice / bob）。策略路径、审计路径、会话、手动写审计和 demo 收在「高级」里。点「浏览」会打开系统文件框（需要 tkinter）。
